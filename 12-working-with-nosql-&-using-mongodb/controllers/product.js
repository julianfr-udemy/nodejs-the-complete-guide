...
exports.get = async (req, res, next) => {
  try {
    res.render('product/list', { title: 'Products', path: '/products', products: await Product.fetchAll() });
  } catch (error) {
    console.log(error);
  }
};
...
exports.getAdmin = async (req, res, next) => {
  try {
    res.render('product/admin', { title: 'Admin Product', path: '/products/admin', products: await Product.fetchAll() });
  } catch (error) {
    console.log(error);
  }
};
...
exports.put = async (req, res, next) => {
  try {
    const product = new Product({ ...req.body, _id: req.params.id, user: req.user._id });
